...
(defmacro get-max-write-operations-per-hour
  "https://developer.chrome.com/apps/bookmarks#property-MAX_WRITE_OPERATIONS_PER_HOUR."
  ([] (gen-call :property ::max-write-operations-per-hour &form)))

(defmacro get-max-sustained-write-operations-per-minute
  "https://developer.chrome.com/apps/bookmarks#property-MAX_SUSTAINED_WRITE_OPERATIONS_PER_MINUTE."
...
(defmacro tap-on-created-events
  "Fired when a bookmark or folder is created.
   ...
   Events will be put on the |channel| with signature [::on-created [id bookmark]] where:
     ...
     |id| - https://developer.chrome.com/apps/bookmarks#property-onCreated-id.
     |bookmark| - https://developer.chrome.com/apps/bookmarks#property-onCreated-bookmark.

   Note: |args| will be passed as additional parameters into Chrome event's .addListener call.

   https://developer.chrome.com/apps/bookmarks#event-onCreated."
...
(defmacro tap-on-removed-events
  "Fired when a bookmark or folder is removed.  When a folder is removed recursively, a single notification is fired for the
   ...
   Events will be put on the |channel| with signature [::on-removed [id remove-info]] where:
     ...
     |id| - https://developer.chrome.com/apps/bookmarks#property-onRemoved-id.
     |remove-info| - https://developer.chrome.com/apps/bookmarks#property-onRemoved-removeInfo.

   Note: |args| will be passed as additional parameters into Chrome event's .addListener call.

   https://developer.chrome.com/apps/bookmarks#event-onRemoved."
...
(defmacro tap-on-changed-events
  "Fired when a bookmark or folder changes.  Note: Currently, only title and url changes trigger this.
   ...
   Events will be put on the |channel| with signature [::on-changed [id change-info]] where:
     ...
     |id| - https://developer.chrome.com/apps/bookmarks#property-onChanged-id.
     |change-info| - https://developer.chrome.com/apps/bookmarks#property-onChanged-changeInfo.

   Note: |args| will be passed as additional parameters into Chrome event's .addListener call.

   https://developer.chrome.com/apps/bookmarks#event-onChanged."
...
(defmacro tap-on-moved-events
  "Fired when a bookmark or folder is moved to a different parent folder.
   ...
   Events will be put on the |channel| with signature [::on-moved [id move-info]] where:
     ...
     |id| - https://developer.chrome.com/apps/bookmarks#property-onMoved-id.
     |move-info| - https://developer.chrome.com/apps/bookmarks#property-onMoved-moveInfo.

   Note: |args| will be passed as additional parameters into Chrome event's .addListener call.

   https://developer.chrome.com/apps/bookmarks#event-onMoved."
...
(defmacro tap-on-children-reordered-events
  "Fired when the children of a folder have changed their order due to the order being sorted in the UI.  This is not called
   ...
   Events will be put on the |channel| with signature [::on-children-reordered [id reorder-info]] where:
     ...
     |id| - https://developer.chrome.com/apps/bookmarks#property-onChildrenReordered-id.
     |reorder-info| - https://developer.chrome.com/apps/bookmarks#property-onChildrenReordered-reorderInfo.

   Note: |args| will be passed as additional parameters into Chrome event's .addListener call.

   https://developer.chrome.com/apps/bookmarks#event-onChildrenReordered."
...
(defmacro tap-on-import-began-events
  "Fired when a bookmark import session is begun.  Expensive observers should ignore onCreated updates until onImportEnded is
   ...
   https://developer.chrome.com/apps/bookmarks#event-onImportBegan."
...
(defmacro tap-on-import-ended-events
  "Fired when a bookmark import session is ended.
   ...
   https://developer.chrome.com/apps/bookmarks#event-onImportEnded."
  ([channel & args] (apply gen-call :event ::on-import-ended &form channel args)))

; -- convenience ------------------------------------------------------------------------------------------------------------

(defmacro tap-all-events
  "Taps all valid non-deprecated events in chromex.app.bookmarks namespace."
  [chan]
