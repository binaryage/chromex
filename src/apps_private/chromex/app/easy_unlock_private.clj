...
(defmacro get-strings
  "Gets localized strings required to render the API.
   ...
   Signature of the result value put on the channel is [strings] where:
     ...
     |strings| - See https://developer.chrome.com/extensions/easyUnlockPrivate#property-callback-strings.
   
   See https://developer.chrome.com/extensions/easyUnlockPrivate#method-getStrings."
...
(defmacro generate-ec-p256-key-pair
  "Generates a ECDSA key pair for P256 curve. Public key will be in format recognized by secure wire transport protocol used
   ...
   This function returns a core.async channel which eventually receives a result value and closes.
   Signature of the result value put on the channel is [public_key private_key] where:
   
     |public_key| - See https://developer.chrome.com/extensions/easyUnlockPrivate#property-callback-public_key.
     |private_key| - See https://developer.chrome.com/extensions/easyUnlockPrivate#property-callback-private_key.
   
   See https://developer.chrome.com/extensions/easyUnlockPrivate#method-generateEcP256KeyPair."
...
(defmacro perform-ecdh-key-agreement
  "Given a private key and a public ECDSA key from different asymetric key pairs, it generates a symetric encryption key using
   ...
   Signature of the result value put on the channel is [data] where:
     ...
     |data| - See https://developer.chrome.com/extensions/easyUnlockPrivate#property-callback-data.
   
   See https://developer.chrome.com/extensions/easyUnlockPrivate#method-performECDHKeyAgreement."
...
(defmacro create-secure-message
  "Creates a secure, signed message in format used by Easy Unlock app to establish secure communication channel over unsecure
   ...
   See https://developer.chrome.com/extensions/easyUnlockPrivate#method-createSecureMessage."
...
(defmacro unwrap-secure-message
  "Authenticates and, if needed, decrypts a secure message. The message is in the same format as the one created by
   ...
   See https://developer.chrome.com/extensions/easyUnlockPrivate#method-unwrapSecureMessage."
...
(defmacro seek-bluetooth-device-by-address
  "Connects to the SDP service on a device, given just the device's Bluetooth address. This function is useful as a faster
   ...
   See https://developer.chrome.com/extensions/easyUnlockPrivate#method-seekBluetoothDeviceByAddress."
...
(defmacro connect-to-bluetooth-service-insecurely
  "Connects the socket to a remote Bluetooth device over an insecure connection, i.e. a connection that requests no bonding
   ...
   See https://developer.chrome.com/extensions/easyUnlockPrivate#method-connectToBluetoothServiceInsecurely."
...
(defmacro update-screenlock-state
  "Updates the screenlock state to reflect the Easy Unlock app state.
     ...
     |state| - See https://developer.chrome.com/extensions/easyUnlockPrivate#property-updateScreenlockState-state.
   
   This function returns a core.async channel which eventually receives a result value and closes.
   Signature of the result value put on the channel is [].
   
   See https://developer.chrome.com/extensions/easyUnlockPrivate#method-updateScreenlockState."
...
(defmacro set-permit-access
  "Saves the permit record for the local device.
   ...
   See https://developer.chrome.com/extensions/easyUnlockPrivate#method-setPermitAccess."
...
(defmacro get-permit-access
  "Gets the permit record for the local device.
   ...
   This function returns a core.async channel which eventually receives a result value and closes.
   Signature of the result value put on the channel is [permitAccess] where:
   
     |permitAccess| - See https://developer.chrome.com/extensions/easyUnlockPrivate#property-callback-permitAccess.
   
   See https://developer.chrome.com/extensions/easyUnlockPrivate#method-getPermitAccess."
...
(defmacro clear-permit-access
  "Clears the permit record for the local device.
   ...
   See https://developer.chrome.com/extensions/easyUnlockPrivate#method-clearPermitAccess."
...
(defmacro set-remote-devices
  "Saves the remote device list.
   ...
   See https://developer.chrome.com/extensions/easyUnlockPrivate#method-setRemoteDevices."
...
(defmacro get-remote-devices
  "Gets the remote device list.
   ...
   Signature of the result value put on the channel is [devices] where:
     ...
     |devices| - See https://developer.chrome.com/extensions/easyUnlockPrivate#property-callback-devices.
   
   See https://developer.chrome.com/extensions/easyUnlockPrivate#method-getRemoteDevices."
...
(defmacro get-sign-in-challenge
  "Gets the sign-in challenge for the current user.
   ...
   This function returns a core.async channel which eventually receives a result value and closes.
   Signature of the result value put on the channel is [challenge signedNonce] where:
   
     |challenge| - The sign in challenge to be used when signing in the user     currently associated with the Easy Unlock
                   service.
     |signedNonce| - Nonce signed by Chrome OS TPM, provided as an argument to     the |getSignInChallenge()| function and
                     signed by the TPM key     associated with the user.
   
   See https://developer.chrome.com/extensions/easyUnlockPrivate#method-getSignInChallenge."
...
(defmacro try-sign-in-secret
  "Tries to sign-in the current user with a secret obtained by decrypting the sign-in challenge. Check
   chrome.runtime.lastError for failures. Upon success, the user session will be started.
     ...
     |signInSecret| - See https://developer.chrome.com/extensions/easyUnlockPrivate#property-trySignInSecret-signInSecret.
   
   This function returns a core.async channel which eventually receives a result value and closes.
   Signature of the result value put on the channel is [].
   
   See https://developer.chrome.com/extensions/easyUnlockPrivate#method-trySignInSecret."
...
(defmacro get-user-info
  "Retrieves information about the user associated with the Easy unlock service.
   ...
   Signature of the result value put on the channel is [users] where:
     ...
     |users| - See https://developer.chrome.com/extensions/easyUnlockPrivate#property-callback-users.
   
   See https://developer.chrome.com/extensions/easyUnlockPrivate#method-getUserInfo."
...
(defmacro get-connection-info
  "Gets the connection info for the Bluetooth device identified by deviceAddress.
     ...
     |deviceAddress| - See
                       https://developer.chrome.com/extensions/easyUnlockPrivate#property-getConnectionInfo-deviceAddress.
   
   This function returns a core.async channel which eventually receives a result value and closes.
   Signature of the result value put on the channel is [rssi transmit_power max_transmit_power] where:
     ...
     |transmit_power| - See https://developer.chrome.com/extensions/easyUnlockPrivate#property-callback-transmit_power.
   |max_transmit_power| - See
   https://developer.chrome.com/extensions/easyUnlockPrivate#property-callback-max_transmit_power.
   
   See https://developer.chrome.com/extensions/easyUnlockPrivate#method-getConnectionInfo."
...
(defmacro show-error-bubble
  "Shows an error bubble with the given |message|, anchored to an edge of the given |anchorRect| -- typically the right edge,
   ...
   given |link_target| URL.
     ...
     |message| - See https://developer.chrome.com/extensions/easyUnlockPrivate#property-showErrorBubble-message.
     |link_range| - See https://developer.chrome.com/extensions/easyUnlockPrivate#property-showErrorBubble-link_range.
     |link_target| - See https://developer.chrome.com/extensions/easyUnlockPrivate#property-showErrorBubble-link_target.
     |anchorRect| - See https://developer.chrome.com/extensions/easyUnlockPrivate#property-showErrorBubble-anchorRect.
   
   See https://developer.chrome.com/extensions/easyUnlockPrivate#method-showErrorBubble."
...
(defmacro hide-error-bubble
  "Hides the currently visible error bubble, if there is one.
   ...
   See https://developer.chrome.com/extensions/easyUnlockPrivate#method-hideErrorBubble."
...
(defmacro set-auto-pairing-result
  "Sets the result of auto pairing triggered from onStartAutoPairing event. If auto pairing is completed successfully,
   ...
   |result.success| is set to false with an optional error message to be displayed to the user.
     ...
     |result| - See https://developer.chrome.com/extensions/easyUnlockPrivate#property-setAutoPairingResult-result.
   
   This function returns a core.async channel which eventually receives a result value and closes.
   Signature of the result value put on the channel is [].
   
   See https://developer.chrome.com/extensions/easyUnlockPrivate#method-setAutoPairingResult."
...
(defmacro find-setup-connection
  "Finds and connects the remote BLE device that is advertising: |setupServiceUUID|. Returns when a connection is found or
   |timeOut| seconds have elapsed.
     ...
     |setupServiceUuid| - See
                          https://developer.chrome.com/extensions/easyUnlockPrivate#property-findSetupConnection-setupService
                          Uuid.
     |timeOut| - See https://developer.chrome.com/extensions/easyUnlockPrivate#property-findSetupConnection-timeOut.
   
   This function returns a core.async channel which eventually receives a result value and closes.
   Signature of the result value put on the channel is [connectionId deviceAddress] where:
   
     |connectionId| - The identifier of the connection found. To be used in future calls refering to this connection.
     |deviceAddress| - The Bluetooth address of the remote device.
   
   See https://developer.chrome.com/extensions/easyUnlockPrivate#method-findSetupConnection."
...
(defmacro setup-connection-status
  "Returns the status of the connection with |connectionId|.
     ...
     |connectionId| - See
                      https://developer.chrome.com/extensions/easyUnlockPrivate#property-setupConnectionStatus-connectionId.
   ...
   See https://developer.chrome.com/extensions/easyUnlockPrivate#method-setupConnectionStatus."
...
(defmacro setup-connection-disconnect
  "Disconnects the connection with |connectionId|.
     ...
     |connectionId| - See
                      https://developer.chrome.com/extensions/easyUnlockPrivate#property-setupConnectionDisconnect-connection
                      Id.
   
   This function returns a core.async channel which eventually receives a result value and closes.
   Signature of the result value put on the channel is [].
   
   See https://developer.chrome.com/extensions/easyUnlockPrivate#method-setupConnectionDisconnect."
...
(defmacro setup-connection-send
  "Sends |data| through the connection with |connnectionId|.
     ...
     |connectionId| - See
                      https://developer.chrome.com/extensions/easyUnlockPrivate#property-setupConnectionSend-connectionId.
     |data| - See https://developer.chrome.com/extensions/easyUnlockPrivate#property-setupConnectionSend-data.
   
   This function returns a core.async channel which eventually receives a result value and closes.
   Signature of the result value put on the channel is [].
   
   See https://developer.chrome.com/extensions/easyUnlockPrivate#method-setupConnectionSend."
...
(defmacro setup-connection-get-device-address
  "Gets the Bluetooth address of the connection with |connectionId
     ...
     |connectionId| - See
                      https://developer.chrome.com/extensions/easyUnlockPrivate#property-setupConnectionGetDeviceAddress-conn
                      ectionId.
   
   This function returns a core.async channel which eventually receives a result value and closes.
   Signature of the result value put on the channel is [deviceAddress] where:
   
     |deviceAddress| - The bluetooth address of the connection with  |connectionId|.
   
   See https://developer.chrome.com/extensions/easyUnlockPrivate#method-setupConnectionGetDeviceAddress."
...
(defmacro tap-on-user-info-updated-events
  "Event fired when the data for the user currently associated with Easy unlock service is updated. |userInfo| The updated
   ...
   See https://developer.chrome.com/extensions/easyUnlockPrivate#event-onUserInfoUpdated."
...
(defmacro tap-on-start-auto-pairing-events
  "Event fired at the end of Easy bootstrap to start auto pairing so that a proper cryptohome key could be generated for the
   ...
   See https://developer.chrome.com/extensions/easyUnlockPrivate#event-onStartAutoPairing."
...
(defmacro tap-on-connection-status-changed-events
  "Event fired when |connectionId| change status.
   ...
   See https://developer.chrome.com/extensions/easyUnlockPrivate#event-onConnectionStatusChanged."
...
(defmacro tap-on-data-received-events
  "Event fired when |connectionId| receives |data|.
   ...
   See https://developer.chrome.com/extensions/easyUnlockPrivate#event-onDataReceived."
...
(defmacro tap-on-send-completed-events
  "Event fired when |connectionId| sends |data|. |success| is true if the send operation was successful.
   ...
   See https://developer.chrome.com/extensions/easyUnlockPrivate#event-onSendCompleted."
