...
(defmacro update
  ...
  (gen-call :function ::update (meta &form) id update-properties))
...
(defmacro remove
  ...
  (gen-call :function ::remove (meta &form) menu-item-id))
...
(defmacro remove-all
  ...
  [#_callback]
  (gen-call :function ::remove-all (meta &form)))
...
(def api-table
  {:namespace "chrome.contextMenus",
   :since "6",
   :properties
   [{:id ::action-menu-top-level-limit,
