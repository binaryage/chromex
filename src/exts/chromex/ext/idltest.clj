...
(defmacro send-array-buffer
  "Functions for testing binary data request/response parameters. The first two just return back the bytes they were passed in
   an array.
     ...
     |input| - See https://developer.chrome.com/extensions/idltest#property-sendArrayBuffer-input.
   
   This function returns a core.async channel which eventually receives a result value and closes.
   Signature of the result value put on the channel is [array] where:
   
     |array| - See https://developer.chrome.com/extensions/idltest#property-cb-array.
   
   See https://developer.chrome.com/extensions/idltest#method-sendArrayBuffer."
...
(defmacro send-array-buffer-view
  "TODO(asargent) - we currently can't have [instanceOf=ArrayBufferView], I think because ArrayBufferView isn't an
   ...
   Uint8Array, Uint16Array, Float32Array, etc.
     ...
     |input| - See https://developer.chrome.com/extensions/idltest#property-sendArrayBufferView-input.
   ...
   See https://developer.chrome.com/extensions/idltest#method-sendArrayBufferView."
...
(defmacro get-array-buffer
  "
   ...
   Signature of the result value put on the channel is [buffer] where:
     ...
     |buffer| - See https://developer.chrome.com/extensions/idltest#property-cb-buffer.
   
   See https://developer.chrome.com/extensions/idltest#method-getArrayBuffer."
...
(defmacro nocompile-func
  "This function should not have C++ code autogenerated (the variable name |switch| should cause compile errors if it does).
   But the name should get defined and made visible from within extensions/apps code.
     ...
     |switch| - See https://developer.chrome.com/extensions/idltest#property-nocompileFunc-switch.
   
   See https://developer.chrome.com/extensions/idltest#method-nocompileFunc."
...
(defmacro nodefine-func
  "This function should not have C++ code autogenerated (the variable name |switch| should cause compile errors if it does).
   The name should also never be defined in Javascript and invisible to extensions/apps code.
     ...
     |switch| - See https://developer.chrome.com/extensions/idltest#property-nodefineFunc-switch.
   
   See https://developer.chrome.com/extensions/idltest#method-nodefineFunc."
