...
(defmacro execute-task
  ...
  ([task-id entries #_callback] (gen-call :function ::execute-task (meta &form) task-id entries)))
...
(defmacro set-default-task
  ...
  ([task-id entries mime-types #_callback] (gen-call :function ::set-default-task (meta &form) task-id entries mime-types)))
...
(defmacro get-file-tasks
  ...
  ([entries #_callback] (gen-call :function ::get-file-tasks (meta &form) entries)))
...
(defmacro get-mime-type
  ...
  ([entry #_callback] (gen-call :function ::get-mime-type (meta &form) entry)))
...
(defmacro add-file-watch
  ...
  ([entry #_callback] (gen-call :function ::add-file-watch (meta &form) entry)))
...
(defmacro remove-file-watch
  ...
  ([entry #_callback] (gen-call :function ::remove-file-watch (meta &form) entry)))
...
(defmacro grant-access
  "Requests granting R/W permissions for the passed entries. It's a best effort operation. Some files may not be
   ...
   be accessed. |callback
     ...
     |callback| - Callback that does not take arguments."
  ([entry-urls #_callback] (gen-call :function ::grant-access (meta &form) entry-urls)))
...
(defmacro select-files
  "Selects multiple files. |selectedPaths| Array of selected paths |shouldReturnLocalPath| true if paths need to be
   resolved to local paths. |callback
     ...
     |callback| - Callback that does not take arguments."
  ([selected-paths should-return-local-path #_callback] (gen-call :function ::select-files (meta &form) selected-paths should-return-local-path)))
...
(defmacro select-file
  "Selects a file. |selectedPath| A selected path |index| Index of Filter |forOpening| true if paths are selected for
   opening. false if for saving. |shouldReturnLocalPath| true if paths need to be resolved to local paths. |callback
     ...
     |callback| - Callback that does not take arguments."
  ([selected-path index for-opening should-return-local-path #_callback] (gen-call :function ::select-file (meta &form) selected-path index for-opening should-return-local-path)))
...
(defmacro get-entry-properties
  ...
  ([entries names #_callback] (gen-call :function ::get-entry-properties (meta &form) entries names)))
...
(defmacro pin-drive-file
  "Pins/unpins a Drive file in the cache. |entry| Entry to pin/unpin. |pin| Pass true to pin the file. |callback
   ...
   Completion callback. 'runtime.lastError' will be set if     there was an error.
     ...
     |callback| - Callback that does not take arguments."
  ([entry pin #_callback] (gen-call :function ::pin-drive-file (meta &form) entry pin)))
...
(defmacro resolve-isolated-entries
  ...
  ([entries #_callback] (gen-call :function ::resolve-isolated-entries (meta &form) entries)))
...
(defmacro add-mount
  ...
  ([source #_callback] (gen-call :function ::add-mount (meta &form) source)))

(defmacro remove-mount
  "Unmounts a mounted resource. |volumeId| An ID of the volume."
  ([volume-id] (gen-call :function ::remove-mount (meta &form) volume-id)))
...
(defmacro get-volume-metadata-list
  ...
  ([#_callback] (gen-call :function ::get-volume-metadata-list (meta &form))))
...
(defmacro cancel-file-transfers
  "Cancels ongoing file transfers for selected files. |entries| Array of files for which ongoing transfer should be
   canceled. |callback| Completion callback of the cancel.
     ...
     |callback| - Callback that does not take arguments."
  ([entries #_callback] (gen-call :function ::cancel-file-transfers (meta &form) entries)))

(defmacro cancel-all-file-transfers
  "Cancels all ongoing file transfers. |callback| Completion callback of the cancel.
   
     |callback| - Callback that does not take arguments."
  ([#_callback] (gen-call :function ::cancel-all-file-transfers (meta &form))))
...
(defmacro start-copy
  ...
  ([entry parent-entry new-name #_callback] (gen-call :function ::start-copy (meta &form) entry parent-entry new-name)))
...
(defmacro cancel-copy
  "Cancels the running copy task. |copyId| ID of the copy task to be cancelled. |callback| Completion callback of the
   cancel.
     ...
     |callback| - Callback that does not take arguments."
  ([copy-id #_callback] (gen-call :function ::cancel-copy (meta &form) copy-id)))
...
(defmacro get-size-stats
  ...
  ([volume-id #_callback] (gen-call :function ::get-size-stats (meta &form) volume-id)))

(defmacro format-volume
  "Formats a mounted volume. |volumeId| ID of the volume to be formatted."
  ([volume-id] (gen-call :function ::format-volume (meta &form) volume-id)))

(defmacro get-preferences
  "Retrieves file manager preferences. |callback|"
  ([#_callback] (gen-call :function ::get-preferences (meta &form))))

(defmacro set-preferences
  "Sets file manager preferences. |changeInfo|"
  ([change-info] (gen-call :function ::set-preferences (meta &form) change-info)))
...
(defmacro search-drive
  ...
  ([search-params #_callback] (gen-call :function ::search-drive (meta &form) search-params)))

(defmacro search-drive-metadata
  "Performs drive metadata search. |searchParams| |callback|"
  ([search-params #_callback] (gen-call :function ::search-drive-metadata (meta &form) search-params)))
...
(defmacro search-files-by-hashes
  "Search files in the volume having |volumeId| by using |hashList|. sub-directories) the given |targetDirectoryUrl|.
     ...
     |callback| - |urls| The map of hash and array of FileEntry's URL. The array can be empty if the corresponding
                  file is not found."
  ([volume-id hash-list #_callback] (gen-call :function ::search-files-by-hashes (meta &form) volume-id hash-list)))
...
(defmacro zip-selection
  ...
  ([parent-entry entries dest-name #_callback] (gen-call :function ::zip-selection (meta &form) parent-entry entries dest-name)))

(defmacro get-drive-connection-state
  "Retrieves the state of the current drive connection. |callback|"
  ([#_callback] (gen-call :function ::get-drive-connection-state (meta &form))))
...
(defmacro validate-path-name-length
  ...
  ([parent-entry name #_callback] (gen-call :function ::validate-path-name-length (meta &form) parent-entry name)))

(defmacro zoom
  "Changes the zoom factor of the Files.app. |operation| Zooming mode."
  ([operation] (gen-call :function ::zoom (meta &form) operation)))
...
(defmacro request-access-token
  ...
  ([refresh #_callback] (gen-call :function ::request-access-token (meta &form) refresh)))
...
(defmacro request-web-store-access-token
  "Requests a Webstore API OAuth2 access token. |callback
     ...
     |callback| - |accessToken| OAuth2 access token, or an empty string if failed to fetch."
  ([#_callback] (gen-call :function ::request-web-store-access-token (meta &form))))
...
(defmacro get-share-url
  ...
  ([entry #_callback] (gen-call :function ::get-share-url (meta &form) entry)))
...
(defmacro get-download-url
  "Requests a download url to download the file contents. |entry| The entry to download. |callback
     ...
     |callback| - |url| Result url."
  ([entry #_callback] (gen-call :function ::get-download-url (meta &form) entry)))
...
(defmacro request-drive-share
  "Requests to share drive files. |entry| Entry to be shared. |shareType| Type of access that is getting granted.
     ...
     |callback| - Callback that does not take arguments."
  ([entry share-type #_callback] (gen-call :function ::request-drive-share (meta &form) entry share-type)))
...
(defmacro compute-checksum
  ...
  ([entry #_callback] (gen-call :function ::compute-checksum (meta &form) entry)))

(defmacro is-uma-enabled
  "Is UMA enabled?
   
     |callback| - |result| Boolean result returned by the invoked function."
  ([#_callback] (gen-call :function ::is-uma-enabled (meta &form))))

(defmacro set-entry-tag
  "Sets a tag on a file or a directory. Only Drive files are supported.
   
     |callback| - Callback that does not take arguments."
  ([entry visibility key value #_callback] (gen-call :function ::set-entry-tag (meta &form) entry visibility key value)))

(defmacro is-piex-loader-enabled
  "Returns if Piex loader is enabled.
   
     |callback| - |result| Boolean result returned by the invoked function."
  ([#_callback] (gen-call :function ::is-piex-loader-enabled (meta &form))))
...
(defmacro get-providing-extensions
  ...
  ([#_callback] (gen-call :function ::get-providing-extensions (meta &form))))
...
(defmacro add-provided-file-system
  "Requests adding a new provided file system. If not possible, then an error via chrome.runtime.lastError is
   returned.
     ...
     |callback| - Callback that does not take arguments."
  ([extension-id #_callback] (gen-call :function ::add-provided-file-system (meta &form) extension-id)))
...
(defmacro configure-volume
  "Requests configuring an existing volume. If not possible, then returns an error via chrome.runtime.lastError.
     ...
     |callback| - Callback that does not take arguments."
  ([volume-id #_callback] (gen-call :function ::configure-volume (meta &form) volume-id)))
...
(defmacro get-custom-actions
  ...
  ([entries #_callback] (gen-call :function ::get-custom-actions (meta &form) entries)))
...
(defmacro execute-custom-action
  "Executes a custom action for a set of entries. If not possible, then an error via chrome.runtime.lastError is
   returned.
     ...
     |callback| - Callback that does not take arguments."
  ([entries action-id #_callback] (gen-call :function ::execute-custom-action (meta &form) entries action-id)))

; -- events ---------------------------------------------------------------------------------------------------------

(defmacro tap-on-mount-completed-events ([channel] (gen-call :event ::on-mount-completed (meta &form) channel)))

(defmacro tap-on-file-transfers-updated-events ([channel] (gen-call :event ::on-file-transfers-updated (meta &form) channel)))

(defmacro tap-on-copy-progress-events ([channel] (gen-call :event ::on-copy-progress (meta &form) channel)))

(defmacro tap-on-directory-changed-events ([channel] (gen-call :event ::on-directory-changed (meta &form) channel)))

(defmacro tap-on-preferences-changed-events ([channel] (gen-call :event ::on-preferences-changed (meta &form) channel)))

(defmacro tap-on-drive-connection-status-changed-events ([channel] (gen-call :event ::on-drive-connection-status-changed (meta &form) channel)))

(defmacro tap-on-device-changed-events ([channel] (gen-call :event ::on-device-changed (meta &form) channel)))

(defmacro tap-on-drive-sync-error-events ([channel] (gen-call :event ::on-drive-sync-error (meta &form) channel)))
