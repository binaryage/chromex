...
(defmacro get-device-info
  ...
  [service-name #_callback]
  (gen-call :function ::get-device-info (meta &form) service-name))
...
(defmacro create-session
  "Create new pairing.
     ...
     |callback| - Called when device starts to establish a secure session. If |status| is |success| app should call
                  |startPairing|."
  [service-name #_callback]
  (gen-call :function ::create-session (meta &form) service-name))
...
(defmacro start-pairing
  ...
  [session-id pairing-type #_callback]
  (gen-call :function ::start-pairing (meta &form) session-id pairing-type))
...
(defmacro confirm-code
  "Confirm pairing code. Should be called after |startPairing|.
     ...
     |callback| - Generic callback for session calls, with status only."
  [session-id code #_callback]
  (gen-call :function ::confirm-code (meta &form) session-id code))
...
(defmacro send-message
  ...
  [session-id api input #_callback]
  (gen-call :function ::send-message (meta &form) session-id api input))
...
(defmacro terminate-session
  "Terminate the session with the device.
     ...
     |sessionId| - The ID of the session created with |establishSession|."
  [session-id]
  (gen-call :function ::terminate-session (meta &form) session-id))
