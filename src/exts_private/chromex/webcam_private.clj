...
(defmacro open-serial-webcam
  ...
  [path protocol #_callback]
  (gen-call :function ::open-serial-webcam (meta &form) path protocol))

(defmacro close-webcam
  "Close a serial port connection to a webcam."
  [webcam-id]
  (gen-call :function ::close-webcam (meta &form) webcam-id))

(defmacro get [webcam-id #_callback]
  (gen-call :function ::get (meta &form) webcam-id))

(defmacro set [webcam-id config]
  (gen-call :function ::set (meta &form) webcam-id config))
...
(defmacro reset
  ...
  [webcam-id config]
  (gen-call :function ::reset (meta &form) webcam-id config))
